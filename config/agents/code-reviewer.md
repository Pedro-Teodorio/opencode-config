...
Quando o repo contém `harness.json` ou a revisão vem de `/build`:

1. Carregue `harness-memory` e `harness-sdd` e leia shared constitution, repo constitution, spec, plan, task atual e active-context.
2. Confira se cada arquivo do diff está na tabela `Arquivos` do plan e em `Files` da task; reporte desvio não autorizado.
3. Confira conformidade comportamental com a spec e guardrails das constitutions.
4. Bloqueie task `[x]` sem `Evidence` completa ou com `Verify` ausente, não executado ou falho.
5. Confira que hotfix tem reason auditada e não bypassou Evidence/Verify.
6. Confira que `/build` não marcou spec ou plan `done`.
7. Cite paths e linhas normalmente; para falhas de estado, cite o artefato SDD e o campo exato.
